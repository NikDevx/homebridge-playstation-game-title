...
export class PlaystationAccessory {
    ...
    private startTitleUpdateLoop() {
        ...
        this.titleUpdateInterval = setInterval(() => {
            if (this.deviceInformation.status !== DeviceStatus.AWAKE) {
                this.platform.log.debug(`[${this.deviceInformation.id}] Console not awake, skipping title check`);
                return;
            }
            ...
            get_title.on('close', () => {
                ...
                if (
                    ...
                    newTitle.includes('Console offline')
                ) {
                    this.platform.log.debug(`🔇 Ignored bad title: ${newTitle}`);
                    return;
                }
    ...
    private updateGameTitleNow() {
        if (this.deviceInformation.status !== DeviceStatus.AWAKE) return;
        ...
        get_title.on('close', () => {
            ...
            if (newTitle !== this.lastTitle && this.dynamicTitleSource) {
                this.lastTitle = newTitle;
                const safeTitle = newTitle.substring(0, 63);
                this.platform.log.info(`🎮 Updating title to: ${safeTitle}`);
